...
def cross_correlation(data_type, signals, sr, window_size, stride, plot_peaks=False, n_central_bins=64, index=""):
    ...
    fig, axs = plt.subplots(n_pairs, 1, figsize=(10, 5))
    if n_pairs == 1:
...
class CNN2DDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image_path= os.path.join("plots/", self.data_type, f"plot_{index}.png")
...
class CNN2DDataLoader(DataLoader):
    def __init__(self, data_type, dir, window_size, stride, batch_size=32, shuffle=False, num_workers=0):
        dataset = CNN2DDataset(data_type, dir, window_size, stride)
