...
class SourceCountingDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        source_coordinates = metadata[index]["source_coordinates"]
        num_sources = len(source_coordinates)
        ...
        #Format the number of sources so that it is either 0 or 1
        if(num_sources==0):
            num_sources=0 
        else:
            num_sources=1
        target_tensor = torch.tensor([num_sources], dtype=torch.float32)  # Convert target to tensor

        return gcc_phat_tensor, target_tensor                              
        #returning gcc_phat and number of sources for each sample
        #this is for voice activation
